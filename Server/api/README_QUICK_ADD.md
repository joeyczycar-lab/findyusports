...
### 方法一：网页上添加 ⭐
1. 打开浏览器
2. 访问：`http://localhost:3000/admin/add-venue`
3. 填写信息，点击"添加场地"
4. 完成！
...
| name | sportType | cityCode | address | lng | lat | priceMin | indoor |
|------|-----------|----------|---------|-----|-----|----------|--------|
| 朝阳体育中心篮球场 | basketball | 110000 | 北京市朝阳区朝阳路1号 | 116.45 | 39.92 | 50 | true |
| 工人体育场足球场 | football | 110000 | 北京市朝阳区工人体育场北路 | 116.44 | 39.93 | 200 | false |

**第2步：保存为CSV**

Excel菜单：文件 -> 另存为 -> 选择"CSV UTF-8"格式 -> 保存
...
```bash
cd Server/api
npm run add-venue:csv -- 你的文件名.csv
```
...
|------|------|------|------|
| name | 场地名称 | ✅ | 朝阳体育中心篮球场 |
| sportType | 运动类型 | ✅ | basketball（篮球）或 football（足球） |
| cityCode | 城市代码 | ✅ | 110000（北京） |
| address | 地址 | ❌ | 北京市朝阳区朝阳路1号 |
| lng | 经度 | ✅ | 116.45 |
| lat | 纬度 | ✅ | 39.92 |
| priceMin | 价格 | ❌ | 50 |
| indoor | 是否室内 | ❌ | true（室内）或 false（室外） |
...
1. **批量准备**：用Excel准备所有场地数据
2. **批量导入**：用CSV导入工具一次性导入
3. **上传图片**：在网页上逐个上传场地图片
...
**Q: CSV导入失败？**
- 检查第一行是不是标题行
- 检查必填列（name, sportType, cityCode, lng, lat）都有数据
- 检查坐标是数字，不是文字

**Q: 坐标怎么查？**
- 用高德地图坐标拾取工具：https://lbs.amap.com/tools/picker
...
如果需要更多功能，可以查看详细文档或使用其他方法：

- JSON导入：`npm run add-venue -- --file venues.json`
- 命令行单个添加：`npm run add-venue -- --name "..." --sportType basketball ...`
- 交互式工具：`npm run add-venue:interactive`
