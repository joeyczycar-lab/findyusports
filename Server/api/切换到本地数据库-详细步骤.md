# 切换到本地数据库 - 详细步骤

## 📋 前置检查

### 1. 检查 PostgreSQL 是否已安装

```bash
psql --version
```

**如果显示版本号**：✅ 已安装，跳到步骤 2  
**如果显示 "command not found"**：❌ 需要安装

---

### 2. 安装 PostgreSQL（如果未安装）

```bash
# 安装
brew install postgresql@14

# 启动服务
brew services start postgresql@14

# 等待几秒，然后检查是否运行
pg_isready -U postgres
```

**如果显示 "accepting connections"**：✅ 成功  
**如果显示错误**：等待几秒后重试

---

## 🚀 快速切换（推荐）

### 方法一：使用自动脚本（最简单）

```bash
cd /Users/Zhuanz/Documents/findyusports/Server/api

# 运行设置脚本
./setup-local-db.sh
```

脚本会自动：
- ✅ 检查 PostgreSQL 是否安装
- ✅ 检查 PostgreSQL 是否运行
- ✅ 创建数据库（如果不存在）
- ✅ 备份当前 .env 文件
- ✅ 生成本地数据库配置

然后按照脚本提示操作即可！

---

### 方法二：手动切换（更灵活）

#### 步骤 1：检查 PostgreSQL

```bash
# 检查是否安装
psql --version

# 检查是否运行
pg_isready -U postgres
```

#### 步骤 2：创建数据库（如果不存在）

```bash
# 创建数据库
createdb -U postgres venues

# 验证创建成功
psql -U postgres -l | grep venues
```

#### 步骤 3：备份当前配置

```bash
cd /Users/Zhuanz/Documents/findyusports/Server/api

# 备份 .env 文件
cp .env .env.railway.backup
```

#### 步骤 4：修改 .env 文件

编辑 `Server/api/.env`，注释掉 Railway 配置，添加本地配置：

```bash
# 原来的 Railway 配置（注释掉）
# DATABASE_URL=postgresql://postgres:...@yamanote.proxy.rlwy.net:29122/railway
# DB_SSL=true

# 本地数据库配置（新增）
DB_HOST=localhost
DB_PORT=5432
DB_USER=postgres
DB_PASS=postgres
DB_NAME=venues

# 其他配置保持不变
PORT=4000
JWT_SECRET=8f2c28d25327d90f3751d54f10fd6d03d8d71192682e19389907b168b8123bfe
```

#### 步骤 5：运行数据库迁移

```bash
cd /Users/Zhuanz/Documents/findyusports/Server/api

# 运行所有迁移
npm run migrate:page-view
npm run migrate:add-contact
npm run migrate:add-is-public
npm run migrate:add-district-code
```

#### 步骤 6：启动服务

```bash
npm run dev
```

---

## ✅ 验证是否成功

### 检查服务是否启动

```bash
# 等待几秒，然后检查
curl http://localhost:4000/health
```

**如果返回 `{"status":"ok"}`**：✅ 成功！

### 检查数据库连接

```bash
# 连接数据库
psql -U postgres -d venues

# 查看表
\dt

# 退出
\q
```

---

## 🔄 切换回 Railway 数据库

如果以后想切换回 Railway：

```bash
cd /Users/Zhuanz/Documents/findyusports/Server/api

# 恢复备份
cp .env.railway.backup .env

# 或者手动编辑 .env，恢复 Railway 配置
```

---

## ❓ 常见问题

### Q: 提示 "psql: command not found"
A: PostgreSQL 未安装，运行 `brew install postgresql@14`

### Q: 提示 "could not connect to server"
A: PostgreSQL 未运行，运行 `brew services start postgresql@14`

### Q: 提示 "database already exists"
A: 数据库已存在，直接使用即可，跳过创建步骤

### Q: 迁移失败怎么办？
A: 检查数据库是否创建成功，然后重试迁移命令

### Q: 服务启动后还是连接失败？
A: 检查 `.env` 文件配置是否正确，确保没有拼写错误

---

## 📝 完整命令清单

```bash
# 1. 检查 PostgreSQL
psql --version
pg_isready -U postgres

# 2. 安装（如果需要）
brew install postgresql@14
brew services start postgresql@14

# 3. 创建数据库
createdb -U postgres venues

# 4. 切换到项目目录
cd /Users/Zhuanz/Documents/findyusports/Server/api

# 5. 备份配置
cp .env .env.railway.backup

# 6. 修改 .env（使用编辑器或脚本）
# 注释掉 DATABASE_URL 和 DB_SSL
# 添加本地数据库配置

# 7. 运行迁移
npm run migrate:page-view
npm run migrate:add-contact
npm run migrate:add-is-public
npm run migrate:add-district-code

# 8. 启动服务
npm run dev

# 9. 验证
curl http://localhost:4000/health
```

---

## 🎯 推荐流程

1. **先运行自动脚本**：`./setup-local-db.sh`
2. **按照脚本提示操作**
3. **如果脚本失败，再手动操作**

这样最快最省事！

