...
export async function POST(
  ...
  try {
    ...
    const backendUrl = `${apiBase}/venues/${venueId}/delete`
    
    console.log('🗑️ [API Route] Proxying delete venue request to:', backendUrl)
    ...
    try {
      ...
      if (fetchError.name === 'AbortError') {
        console.error('❌ [API Route] Delete venue request timeout')
        return Response.json(
          { error: { code: 'Timeout', message: '请求超时，请稍后重试' } },
          { status: 408 }
        )
      }
      console.error('❌ [API Route] Failed to proxy delete venue request:', fetchError)
      return Response.json(
    ...
    console.error('❌ [API Route] Unexpected error in delete venue route:', error)
    return Response.json(
