...
export default function MobileDrawer({ open, onClose, children }: Props) {
  ...
  return (
    <>
      ...
      />
      <div
        className={`fixed left-0 right-0 bottom-0 bg-white rounded-t-modal border-t border-border shadow-md lg:hidden transition-transform ${open ? 'translate-y-0' : 'translate-y-full'}`}
        style={{ minHeight: open ? '40vh' : undefined }}
      >
